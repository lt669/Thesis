...
\lstset{language=Java} 
	...
	\subsection{RIR Trimming}
		...
		% The first impulse can be seen at  0.0464 (46ms). As the tweeter is the closest sound source to the microphone and is located 0.22m away (thus taking 0.64ms to reach the receiver), it can be calculated that the impulse starts at 0.04576 (0.0464 - 0.00065). Therefore, to remove 22ms, the file must be trimmed at 0.06776 (67.8ms).

		% Both \ac{RIR}'s show a peak at (2), 0.05405s into the impulse. This can be calculated ($(0.05404-0.0464)\times{340}$) as having travelled 2.65m, which is the distance from the woofer to the floor (1.2m) and back to the receiver $(1.2m + 0.4m = 1.6m)$ total = 1.2 + 1.6 = 2.8. This is not an exact match, however this will be due to some error in the physical measurements of the sound source and receiver placement. The main difference between the two \ac{RIR}'s can be seen by comparing peaks (3) and (4).

		% Peak (3) is shown at 0.06188 $(0.06188-0.04642)*344 = 5.34m$ (0.06188-0.04642)*340 = 5.1m, corresponding to the wall approximately 2.55m away (should be 2m).
		% (4) shows a peak at 0.09654 $(0.06188-0.04642)*344 = 5.34m$
