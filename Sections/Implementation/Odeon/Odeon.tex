...
\subsection{ODEON}
	...
	\label{odeon:materials}
		...
		\paragraph{Initial Materials}
			...
			%-------------Footnote-------------%			
			\footnotetext[1]{The absorption coefficients provided were only available for the 125Hz to 4kHz octave bands, therefore absorption coefficients for the 63Hz and 8kHz octave bands were given a value of 0.1 as a reasonable approximation.}
		...
		\paragraph{RIR Comparison}
			...
			% The reverb time of the real \ac{RIR} used was calculated using a Matlab function taken from \cite{rt60Matlab} and the \textbf{`Global Estimate'} option in Odeon was used to calculate the reverb time of the synthetic \ac{RIR}. The average reverb time of the real \ac{RIR} was approximately 1.4s whereas the average reverb time of the synthetic \ac{RIR} was approximately 1.1s.
		...
		\label{rirlocations}
			...
			% It was also later discovered (see section \nameref{max:latency}) that the latency due to the software used would mean that the beginning of the \ac{RIR}'s would have to be trimmed anyway, avoiding this issue altogether.
	...
	\subsection{RIR Calibration}
		...
		\subsubsection{Matlab}
			...
			%This value was used to multiply each of the samples in the real \ac{RIR}'s to reduce each of the levels to that of the synthetic \ac{RIR}. The full matlab script used can be \href{http://lt669.github.io/code/matlab/html/calibrateAll.html}{calibrateALL.m} can be viewed or found in file 2.
			...
			\begin{figure}[h]
				\begin{center}
					\includegraphics[scale = 0.25]{Sections/Implementation/RealRIRs/images/calibration/CalMan_RIR_edit.png} 
		...
		\subsection{Room Modelling and Synthetic RIR Rendering Summary}
			...
			Though it has been shown that producing 960 \ac{RIR}'s can be done sitting in front of a computer, Odeon is not optimised for such a task. As stated in \nameref{odeon:rendering}, though it is possible to import a simple script to set all of the source and receiver locations, having to join them up one at a time takes the best part of an hour of repetitive point, clicking and dragging. This undesirable software flaw would be simply fixed by adding the option to pair them up within the source and receiver script. In addition to this, though the required distance between the source and receiver in order to prevent erroneous calculations is larger than desired, it is still possible to place them closer together than it is when using physical equipment to measure real \ac{RIR}'s. 
