...
\subsection{ODEON}
	...
	\label{odeon:materials}
		...
		\label{odeonError}
			...
			\footnotetext[1]{\textit{\textbf{Note:} The reason the \ac{RIR} shown in (1) looks different to previously investigated \ac{RIR}'s such as those in figure~\ref{incorrectRIR} is due to (1) being zoomed in, causing it to look greater in amplitude. This was done so it could be compared to (2) - (6) whilst being on the same scale.}}
	...
	\subsection{File Sorting (Extra Grids)}
		...
		As described in section~\nameref{Background:RIRPositions}, user test \#3 requires there to be a grid of \ac{RIR}'s that are separated by a variety of distances. To achieve this, four more \ac{RIR} grids were produced with an increasing distance between the \ac{RIR}'s in each one (2m - 5m). This was done by first finding which \ac{RIR}'s would be included in each of the new grids by starting from the centre of the first grid (position 128) and moving outwards. By doing this, it was possible to find the first \ac{RIR} positions of each new grid. Usingthig, 4 Matlab algorithms were written to extract the appropriate \ac{RIR}'s required for each new gird, and rename them starting from 001 up to the maximum number of \ac{RIR}'s in each grid.

		 For this, the greatest distance between \ac{RIR} locations was found. This was done by starting at the centre of the \ac{RIR} grid (position 128) 
